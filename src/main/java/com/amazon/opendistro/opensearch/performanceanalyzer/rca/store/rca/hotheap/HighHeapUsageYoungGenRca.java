...
package com.amazon.opendistro.opensearch.performanceanalyzer.rca.store.rca.hotheap;
...
import com.amazon.opendistro.opensearch.performanceanalyzer.rca.framework.api.aggregators.SlidingWindow;
import com.amazon.opendistro.opensearch.performanceanalyzer.rca.framework.api.aggregators.SlidingWindowData;
...
import com.amazon.opendistro.opensearch.performanceanalyzer.rca.framework.metrics.RcaVerticesMetrics;
...
public class HighHeapUsageYoungGenRca extends Rca<ResourceFlowUnit<HotResourceSummary>> {
    ...
    public <M extends Metric> HighHeapUsageYoungGenRca(
            final int rcaPeriod,
        ...
        this.lowerBoundThreshold =
                (lowerBoundThreshold >= 0 && lowerBoundThreshold <= 1.0)
                        ...
                        : 1.0;
        this.counter = 0;
        this.minorGcTimeDeque =
                new SlidingWindow<>(PROMOTION_RATE_SLIDING_WINDOW_IN_MINS, TimeUnit.MINUTES);
        this.fullGcTimeDeque =
                new SlidingWindow<>(PROMOTION_RATE_SLIDING_WINDOW_IN_MINS, TimeUnit.MINUTES);
        this.promotionRateThreshold =
                HighHeapUsageYoungGenRcaConfig.DEFAULT_PROMOTION_RATE_THRESHOLD_IN_MB_PER_SEC;
        this.youngGenGcTimeThreshold =
                HighHeapUsageYoungGenRcaConfig.DEFAULT_YOUNG_GEN_GC_TIME_THRESHOLD_IN_MS_PER_SEC;

        this.garbagePromotedDeque =
                new SlidingWindow<>(PROMOTION_RATE_SLIDING_WINDOW_IN_MINS, TimeUnit.MINUTES);
        this.promotionRateDeque =
                new SlidingWindow<SlidingWindowData>(
                        PROMOTION_RATE_SLIDING_WINDOW_IN_MINS, TimeUnit.MINUTES) {
                    /**
                     * always compare the current old gen usage with the usage from the previous
                     * time intervals and the amount of increase is the data that is promoted from
                     * young gen.
                     *
                     * <p>For example : Full GC | old gen(in Mb): 18500 | 18600 | 18700 | 3000 |
                     * 3050 | 3100 data promoted: n/a | 100 | 100 | 0 | 50 | 50 so the promotion
                     * rate within this time window is (100 + 100 + 0 + 50 + 50) / time slice
                     */
                    @Override
                    protected void add(SlidingWindowData e) {
                        if (!windowDeque.isEmpty()
                                && e.getValue() > windowDeque.peekFirst().getValue()) {
                    ...
                    protected void remove(SlidingWindowData e) {
                        if (!windowDeque.isEmpty()
                                && e.getValue() < windowDeque.peekLast().getValue()) {
    ...
    private boolean promotionRateTooHigh(double avgPromotionRate, double modifier) {
        return (!Double.isNaN(avgPromotionRate)
                && avgPromotionRate > promotionRateThreshold * modifier);
    }
     ...
     * @param avgGarbagePromoted the average amount of objects which were promoted to the old
     *     generation then collected from the old generation shortly after. This is a percentage
     *     from 0.0 to 1.0
     ...
     */
    private ResourceFlowUnit<HotResourceSummary> computeFlowUnit(
            double avgPromotionRate,
            double avgYoungGCTime,
            double avgGarbagePromoted,
            double avgFullGCTime) {
        LOG.debug(
                "computing avgPromotionRate = {} , avgGCTime = {}, avgGarbagePromoted = {}, avgFullGcTime = {},",
                avgPromotionRate,
                avgYoungGCTime,
                avgGarbagePromoted,
                avgFullGCTime);
        ...
        if (fullGcTimeTooHigh(avgFullGCTime)) {
            ...
            summary =
                    new HotResourceSummary(
                            ResourceUtil.FULL_GC_PAUSE_TIME,
            ...
            summary =
                    new HotResourceSummary(
                            ResourceUtil.MINOR_GC_PAUSE_TIME,
        ...
        if (unhealthy) {
            LOG.debug(
                    "avgPromotionRate = {} , avgGCTime = {}, avgGarbagePromoted = {}, avgFullGcTime = {},",
                    avgPromotionRate,
                    avgYoungGCTime,
                    avgGarbagePromoted,
                    avgFullGCTime);
    ...
    private void computePromotionHealth(double currOldGen, double fullGcCount, long currTimeStamp) {
        ...
        if (promoted >= 0) {
            ...
            if (garbageReclaimedPct <= 1 && garbageReclaimedPct >= 0) {
                garbagePromotedDeque.next(
                        new SlidingWindowData(currTimeStamp, garbageReclaimedPct));
            }
            // Reset variables
