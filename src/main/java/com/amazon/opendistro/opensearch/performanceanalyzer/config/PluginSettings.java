...
import com.amazon.opendistro.opensearch.performanceanalyzer.ConfigStatus;
...
public class PluginSettings {
    ...
    private static final String BATCH_METRICS_RETENTION_PERIOD_MINUTES =
            "batch-metrics-retention-period-minutes";
    ...
    private void loadMetricsLocationFromConfig() throws ConfigFatalException {
        if (!settings.containsKey(METRICS_LOCATION_KEY)) {
            LOG.info(
                    "Cannot find metrics-location, using default value. {}",
                    METRICS_LOCATION_DEFAULT);
        }
        ...
        try {
            ...
            LOG.error(
                    (Supplier<?>)
                            () ->
                                    new ParameterizedMessage(
                                            "Invalid writer-queue-size. Using default value {}.",
                                            writerQueueSize),
                    e);
        }
    }
    ...
    private void loadMetricsDBFilesCleanupEnabled() {
        ...
        try {
            ...
            // In case of exception, we go with the safe default that the files will always be
            // cleaned up.
    ...
    private void loadBatchMetricsRetentionPeriodMinutesFromConfig() {
        ...
        try {
            long parsedRetentionPeriod =
                    Long.parseLong(settings.getProperty(BATCH_METRICS_RETENTION_PERIOD_MINUTES));
            if (parsedRetentionPeriod < BATCH_METRICS_RETENTION_PERIOD_MINUTES_MIN
                ...
                LOG.error(
                        "{} out of range. Value should be in range [{}, {}]. Using default value {}.",
