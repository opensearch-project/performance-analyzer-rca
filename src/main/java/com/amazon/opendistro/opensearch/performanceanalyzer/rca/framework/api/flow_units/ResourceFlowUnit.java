...
/**
 * ResourceFlowUnit is the flowunit type that is emitted by RCA vertex. It is persisted in the
 * FlowUnit SQLite table
 *
 * <p>The SQL table name : FlowUnit
 *
 * <p>SQL Schema : | ID(primary key) | Timestamp | RCA_Name | state | 1 | 151000 |
 * HighHeapYoungGenRca | healthy
 */
public class ResourceFlowUnit<T extends GenericSummary> extends GenericFlowUnit {
    ...
    public ResourceFlowUnit(
            long timeStamp, ResourceContext context, T summary, boolean persistSummary) {
        super(timeStamp);
    ...
    public static <T extends GenericSummary> ResourceFlowUnit<T> buildFlowUnitFromWrapper(
        ...
        if (message.hasResourceContext()) {
            ResourceContext newContext =
                    ResourceContext.buildResourceContextFromMessage(message.getResourceContext());
            T newSummary = null;
            try {
                    ...
                    case HOTNODESUMMARY:
                        {
                            newSummary =
                                    (T)
                                            HotNodeSummary.buildHotNodeSummaryFromMessage(
                                                    message.getHotNodeSummary());
                            break;
                        }
                    default:
