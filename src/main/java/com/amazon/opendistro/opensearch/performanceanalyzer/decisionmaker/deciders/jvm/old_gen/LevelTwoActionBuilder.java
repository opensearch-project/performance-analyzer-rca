...
import com.amazon.opendistro.opensearch.performanceanalyzer.decisionmaker.actions.configs.ThresholdConfig;
...
import com.amazon.opendistro.opensearch.performanceanalyzer.decisionmaker.deciders.configs.jvm.LevelTwoActionBuilderConfig;
import com.amazon.opendistro.opensearch.performanceanalyzer.decisionmaker.deciders.configs.jvm.OldGenDecisionPolicyConfig;
...
import com.amazon.opendistro.opensearch.performanceanalyzer.rca.store.rca.util.NodeConfigCacheReaderUtil;
 ...
 *
 * <p>if old gen usage(after full gc) is between 75% - 90%, it means the JVM starts to become
 * contended and full GC is not able to free up enough objects to bring down heap usage to below
 * 75%. In this case, we will downsize both caches by a larger step size and starts to downsize
 * queues. The action builder in this level will only try to downsize one type of queue at a time.
 * And action priority is used as a tie breaker if both queues are equally important in terms of
 * reducing heap usage..
 *
 * <p>For field data cache, the lower bound in this bucket is 2% of the heap and for shard request
 * cache / query cache, it will be 1% of the heap
 */
public class LevelTwoActionBuilder {
     ...
     * EWMA size of search/write queue. step here is calculated as {range of queue} / {num of
     * buckets} The queue's lower/upper bound can be configured in rca.conf
     */
    ...
    private void actionPriorityForQueue() {
        ...
        ModifyQueueCapacityAction writeQueueAction =
                queueActionMap.get(ResourceEnum.WRITE_THREADPOOL);
        ModifyQueueCapacityAction searchQueueAction =
                queueActionMap.get(ResourceEnum.SEARCH_THREADPOOL);
        ThresholdConfig<Integer> writeQueueConfig =
                queueActionConfig.getThresholdConfig(ResourceEnum.WRITE_THREADPOOL);
        ThresholdConfig<Integer> searchQueueConfig =
                queueActionConfig.getThresholdConfig(ResourceEnum.SEARCH_THREADPOOL);
        if (writeQueueAction != null && searchQueueAction != null) {
            int writeQueueSizeBucket =
                    bucketization(
    ...
    /**
     * The default priority in this level is 1. downsize both caches simultaneously with larger step
     * size. 2. Allocate size of each queue into buckets and downsize any queue that has higher
     * bucket size. 3. if the bucket size of both queues are equal, the action priority will act as
     * tie breaker and the default setting favors write over search as write is more important in
     * log analytic workload 4. The above default action priority can be overridden by customer
     * settings but customer's acton priority will only affect step 3 above. Step 1 and 2 will be
     * executed regardless of priority action settings
     */
