...
import com.amazon.opendistro.opensearch.performanceanalyzer.rca.framework.api.Resources.State;
import com.amazon.opendistro.opensearch.performanceanalyzer.rca.framework.api.flow_units.ResourceFlowUnit;
import com.amazon.opendistro.opensearch.performanceanalyzer.rca.framework.api.flow_units.ResourceFlowUnit.ResourceFlowUnitFieldValue;
import com.amazon.opendistro.opensearch.performanceanalyzer.rca.framework.api.summaries.temperature.ClusterTemperatureSummary;
import com.amazon.opendistro.opensearch.performanceanalyzer.rca.framework.api.summaries.temperature.CompactNodeSummary;
import com.amazon.opendistro.opensearch.performanceanalyzer.rca.framework.api.summaries.temperature.NodeLevelDimensionalSummary;
import com.amazon.opendistro.opensearch.performanceanalyzer.rca.framework.core.GenericSummary;
import com.amazon.opendistro.opensearch.performanceanalyzer.rca.framework.util.SQLiteQueryUtils;
import com.amazon.opendistro.opensearch.performanceanalyzer.rca.response.RcaResponse;
import com.amazon.opendistro.opensearch.performanceanalyzer.rca.store.rca.temperature.ClusterTemperatureRca;
...
class SQLitePersistor extends PersistorBase {
    ...
    private static final String PRIMARY_KEY_AUTOINCREMENT_POSTFIX =
    ...
    /**
     * This is for efficient lookup of the getter and setter methods for all the persistable Fields
     * of a class. This map can be in-memory and does not need to be re-created during DB file
     * rotations as we don't support dynamic class loading and rotating the DB files should not
     * change the members of the class.
     */
    private Map<Class<?>, Map<java.lang.reflect.Field, GetterSetterPairs>>
            fieldGetterSetterPairsMap;

    private Map<Class<?>, Map<String, GetterSetterPairs>> classFieldNamesToGetterSetterMap;

    // When persisting an object in the DB, is a getter for the Object is annotated with @AColumn
    // and @ATable, then the return type Object is
    // persisted in a a different table and the primary key of the other table is persisted as a
    // pointer in the outer object table.
    ...
    SQLitePersistor(
            String dir,
            String filename,
            String storageFileRetentionCount,
            TimeUnit rotationTime,
            long rotationPeriod)
    ...
    synchronized void createTable(String tableName, List<Field<?>> columns) throws SQLException {
                        ...
                        .column(
                                DSL.field(
                                        getPrimaryKeyColumnName(tableName)
                                                + PRIMARY_KEY_AUTOINCREMENT_POSTFIX))
                        .columns(columns);
        ...
        LOG.debug(
                "Added table '{}' and its columns: '{}' to in-memory registry.",
                tableName,
                columns);
    }
    ...
    synchronized void createTable(
        ...
        try {
            LOG.debug(
                    "Trying to create a summary table: {} that references {}",
                    tableName,
                    referenceTableName);
            ...
            CreateTableConstraintStep constraintStep =
                    create.createTable(tableName)
                            ...
                            .columns(columns)
                            .constraints(
                                    DSL.constraint(foreignKeyField.getName() + "_FK")
                                            .foreignKey(foreignKeyField)
                                            .references(
                                                    referenceTable,
                                                    DSL.field(referenceTablePrimaryKeyFieldName)));
            constraintStep.execute();
    ...
    synchronized int insertRow(String tableName, List<Object> row) throws SQLException {
        ...
        if (columnsForTable == null) {
            LOG.error(
                    "NO columns found for table: {}. Tables: {}, columns: {}",
                    tableName,
                    tableNames,
                    jooqTableColumns);
        ...
        InsertValuesStepN insertValuesStepN =
                create.insertInto(table).columns(columnsForTable).values(row);
    ...
    @Override
    public synchronized <T> @org.checkerframework.checker.nullness.qual.Nullable T read(
            Class<T> clz)
    ...
    public synchronized <T> @org.checkerframework.checker.nullness.qual.Nullable T read(
        ...
        if (rowId == -1) {
            ...
            try {
                ...
                recordList =
                        create.select()
                                .from(tableName)
                                .where(DSL.field(primaryKeyCol, Integer.class).eq(rowId))
                                .fetch();
            } catch (DataAccessException dex) {
                // This is more severe. This would mean that the data corresponding to the outer
                // Object were found but
        ...
        if (recordList.size() != 1) {
            // We always expect one row whether we query for the latest row or we query for a row by
            // the rowID.
    ...
    @Override
    public synchronized <T, E> @org.checkerframework.checker.nullness.qual.Nullable
            List<T> readAllForMaxField(Class<T> clz, String fieldName, Class<E> fieldClz)
                    throws NoSuchMethodException, IllegalAccessException, InvocationTargetException,
                            InstantiationException, DataAccessException {
        String tableName = getTableNameFromClassName(clz);
        ...
        try {
            ...
            recordsWithMaxFieldValue =
                    create.select()
                            .from(tableName)
                            .where(DSL.field(field).eq(create.select(max(field)).from(tableName)))
                            .fetch();
        } catch (DataAccessException dex) {
    ...
    public synchronized <T> @org.checkerframework.checker.nullness.qual.Nullable T readFields(
            Class<T> clz, Record record, String tableName)
            throws NoSuchMethodException, IllegalAccessException, InvocationTargetException,
                    InstantiationException {
        ...
        for (Field<?> jooqField : fields) {
            ...
            if (columnName.startsWith(NESTED_OBJECT_COLUMN_PREFIX)) {
                // If the name of the column starts with the prefix, then it is a reference Column.
                // It means that the value contained in this
                // column references to a particular RowIDs in the table with name same as this
                // column(with prefix removed).
                // There are two forms of this:
                // - It can be a reference to another table, in which case the column is an integer
                // type.
                // - OR it is a collection of RowIDs from multiple tables, in which case the column
                // type a JsonArray as String. e.g.
                //
                ...
                if (jooqField.getType() == String.class) {
                    ...
                    for (JsonElement element : array) {
                        ...
                        if (actualTableClass == null) {
                            throw new IllegalStateException(
                                    "The table name '"
                                            + actualTableName
                                            + "' does not exist in the table to class mapping. But"
                                            + "the database row mentions it: "
                                            + element.toString());
                        }

                        for (JsonElement rowIdElem :
                                jsonObject.get(ROW_IDS_JSON_KEY).getAsJsonArray()) {
                    ...
                    if (fieldNameToGetterSetterMap.get(nestedTableName) == null) {
                        throw new IllegalStateException(
                                "No Field Mapping exist for column name "
                                        + jooqField.getName()
                                        + " of table "
                                        + tableName);
                    }
                    ...
                    // the object
                    // is to be filled with; we call the read method recursively to create the
                    // referenced Object
                    ...
                    throw new IllegalStateException(
                            "ReferenceColumn can be either Integer or String.");
                }
            } else {
    ...
    synchronized <T> void writeImpl(T obj)
            throws IllegalStateException, IllegalArgumentException, InvocationTargetException,
    ...
    private Class<?> getFirstTypeFromParameterizedTypes(ParameterizedType mtype, String name) {
        ...
        if (mTypeArguments.length != 1) {
            throw new IllegalStateException(
                    "Expected list of a single type. Please check field/method: " + name);
        }
    ...
    private void checkPublic(Method method) {
        if (!Modifier.isPublic(method.getModifiers())) {
            throw new IllegalStateException(
                    "Found '" + method.getName() + "'. But it is not public.");
        }
    }
    ...
    private void checkValidType(java.lang.reflect.Field field, Class<?> clz) {
        ...
        if (annotation.length != 1) {
            err.append("A field can have either ")
        ...
        if (annotation[0].annotationType() == RefColumn.class) {
            if (type instanceof ParameterizedType) {
                ...
                if (actualType.isPrimitive() || actualType == String.class) {
                    err.append(
                            "RefColumn can only be used for a user defined class or a Collection of user-defined class NOT "
            ...
            if (type instanceof ParameterizedType) {
                ...
                err.append(
                        "persisting Primitives or Strings as Parameterized Types is not supported.");
                throw new IllegalStateException(err.toString());
            }
        }
    }
    ...
    /**
     * Go over all the fields of the class and then filter out all that are annotated as @AColumn
     * or @ATable. For those fields, try to figure out the getter and setters.
     *
     * @param clz The class whose field registry is to be created.
     * @param <T> The Generic type of the class.
     * @throws IllegalStateException When getters and setters are not found for the field that is
     *     required to be persisted or they exist but are not public.
     */
    ...
    private String getNoGetterSetterExist(
            Class<?> clz, java.lang.reflect.Field field, GetterOrSetter getterOrSetter) {
        String type;
        ...
        sb.append(type)
                ...
                .append(clz.getName())
                .append(
                        "'. Getters are expected to start with 'get' or 'is' and setters are expected to start with 'set' and they are required to"
                                + " end with the name of the field (case insensitive.)");
        return sb.toString();
    }

    private <T> ColumnValuePair writeCollectionReferenceColumn(
            java.lang.reflect.Field field, Method getter, T obj)
    ...
    public synchronized Map<String, Result<Record>> getRecordsForAllTables() {
        ...
        super.tableNames.forEach(table -> results.put(table, getRecords(table)));
        return results;
    }
        ...
        try {
            uniquePersistedRcas =
                    (List<String>)
                            create
                                    .selectDistinct(
                                            ResourceFlowUnitFieldValue.RCA_NAME_FILELD.getField())
    ...
    /**
     * FullTemperatureSummary is not one single Rca, instead it is a conglomeration of temperature
     * across all dimensions. Therefore, it iterates over all the dimensional tables to arrive at
     * that result.
     *
    ...
    private JsonElement constructFullTemperatureProfile() {
            ...
            if (rcaResponseJson == null) {
                ...
                nodeDimensionalSummary
                        ...
                        .addProperty(
                                ...
                                rcaResponseJson
                                        .get(
                                                ResourceFlowUnit.SQL_SCHEMA_CONSTANTS
                                                        .TIMESTAMP_COL_NAME)
                                        .getAsString());
                ...
                if (resp != null && resp.getAsJsonObject().get(summaryName) != null) {
                    ...
                    obj.addProperty(
                            ResourceFlowUnit.SQL_SCHEMA_CONSTANTS.TIMESTAMP_COL_NAME,
                            resp.get(ResourceFlowUnit.SQL_SCHEMA_CONSTANTS.TIMESTAMP_COL_NAME)
                                    .getAsString());
        ...
        if (rcaResponseJson != null) {
            // This is the name of the first dimension ans hence confusing. This element is
            // redundant
    ...
    private synchronized void readSummary(
            GenericSummary upperLevelSummary, int upperLevelPrimaryKey) {
        ...
        for (String nestedTableName : upperLevelSummary.getNestedSummaryTables()) {
            Field<Integer> foreignKeyField =
                    DSL.field(
                            SQLiteQueryUtils.getPrimaryKeyColumnName(upperLevelTable),
                            Integer.class);
            SelectJoinStep<Record> rcaQuery =
                    SQLiteQueryUtils.buildSummaryQuery(
                            create, nestedTableName, upperLevelPrimaryKey, foreignKeyField);
            try {
                Result<Record> recordList = rcaQuery.fetch();
                for (Record record : recordList) {
                    GenericSummary summary =
                            upperLevelSummary.buildNestedSummary(nestedTableName, record);
                    if (summary != null) {
                        Field<Integer> primaryKeyField =
                                DSL.field(
                                        SQLiteQueryUtils.getPrimaryKeyColumnName(
                ...
                // it is totally fine if we fail to read some certain tables as some types of
                // summaries might be missing
                LOG.warn(
                        "Fail to read Summary table : {}, query = {}",
                        nestedTableName,
                        rcaQuery.toString(),
                        de);
    ...
    private synchronized JsonElement readTemperatureProfileRca(String rca) {
        ...
        try {
            ...
            if (rca.equals(ClusterTemperatureRca.TABLE_NAME)) {
                ...
                GenericSummary summary =
                        ClusterTemperatureSummary.buildSummaryFromDatabase(
                                temperatureSummary, create);
