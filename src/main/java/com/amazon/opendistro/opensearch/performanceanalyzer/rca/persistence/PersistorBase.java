...
public abstract class PersistorBase implements Persistable {
    ...
    PersistorBase(
            String dir,
            String filename,
            String dbProtocolString,
            String storageFileRetentionCount,
            TimeUnit fileRotationTimeUnit,
            long fileRotationPeriod)
    ...
    public synchronized String read() {
        // Currently this method only contains a call to readTables() - later could add the part to
        // read
    ...
    //  we will revisit this function to decide what are the inputs of the new API which support
    // range query and
     ...
     *
     * <p>Before, we write anything the flowUnit is not empty and if we are past the rotation
     * period, then we rotate the database file and create a new one.
     *
    ...
    public synchronized <T extends ResourceFlowUnit> void write(Node<?> node, T flowUnit)
        ...
        try {
            ...
            LOG.error(
                    "RCA: Multiple attempts to write the data for table '{}' failed",
                    node.name(),
                    e);
    ...
    public synchronized <T> void write(T obj)
        ...
        try {
            writeImpl(obj);
                ...
                | InvocationTargetException
                | NoSuchMethodException illegalEx) {
            throw illegalEx;
        } catch (SQLException e) {
    ...
    private synchronized void rotateRegisterGarbageThenCreateNewDB(RotationType type)
        ...
        // If we are here that means the tryRotate or the forceRotate didn't throw exception and
        // therefore,
    ...
    private synchronized <T extends ResourceFlowUnit> void writeFlowUnit(
        ...
        try {
            ...
            LOG.info(
                    "RCA: Fail to write to table '{}', creating a new DB file and retrying write/create operation",
                    tableName,
                    e);
            rotateRegisterGarbageThenCreateNewDB(RotationType.FORCE_ROTATE);
            tryWriteFlowUnit(flowUnit, tableName);
    ...
    private synchronized void writeSummary(
        ...
        if (!tableNames.contains(tableName)) {
            LOG.info(
                    "RCA: Summary table '{}' does not exist. Creating one with columns: {}",
                    tableName,
        ...
        for (GenericSummary nestedSummary : summary.getNestedSummaryList()) {
            writeSummary(
                    nestedSummary, tableName, getPrimaryKeyColumnName(tableName), lastPrimaryKey);
        }
    }
