 ...
 *
 * <p>This object is persisted in SQLite table Table name : HotNodeSummary
 *
 * <p>schema : | ID(primary key) | Node ID | Host IP Address | ID in HotClusterSummary(foreign key)
...
public class HotNodeSummary extends GenericSummary {
    ...
    public HotNodeSummaryMessage buildSummaryMessage() {
        final HotNodeSummaryMessage.Builder summaryMessageBuilder =
                HotNodeSummaryMessage.newBuilder();
    ...
    public static HotNodeSummary buildHotNodeSummaryFromMessage(HotNodeSummaryMessage message) {
        ...
        if (message.hasHotResourceSummaryList()) {
            for (int i = 0;
                    i < message.getHotResourceSummaryList().getHotResourceSummaryCount();
                    i++) {
        ...
        if (message.hasHotShardSummaryList()) {
            for (int i = 0; i < message.getHotShardSummaryList().getHotShardSummaryCount(); i++) {
                newSummary.appendNestedSummary(
                        HotShardSummary.buildHotShardSummaryFromMessage(
    ...
    public String toString() {
        return this.nodeID
                ...
                + this.hostAddress
                + " "
                + this.hotResourceSummaryList
                + " "
                + this.hotShardSummaryList;
    }
    ...
    public JsonElement toJson() {
        ...
        summaryObj.addProperty(
                SQL_SCHEMA_CONSTANTS.HOST_IP_ADDRESS_COL_NAME, this.hostAddress.toString());
    ...
    /**
     * return HotResourceSummary and HotShardSummary as a single GenericSummary list. Note that is
     * method is intended to be called by persistor and Json serializer only.
     *
