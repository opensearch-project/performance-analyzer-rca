...
public class Collator extends Decider {
     ...
     */
    private static final int collatorFrequency =
            1; // Measured in terms of number of evaluationIntervalPeriods
    ...
    @VisibleForTesting
    public Collator(
            final ImpactAssessor impactAssessor,
            final Comparator<Action> actionComparator,
            Decider... deciders) {
        super(evalIntervalSeconds, collatorFrequency);
        this.deciders = Arrays.asList(deciders);
    ...
    /**
     * Process all the actions proposed by the deciders and prune them based on their impact
     * vectors.
     *
    ...
    public Decision operate() {
        ...
        // We need to identify and prune conflicting actions based on the overall impact. In order
        // to
        // do that, we re-assess each of the proposed actions with the overall impact assessment in
        // mind. In each such assessment, we ensure the impact of an action aligns with the
        // instance's
        ...
        allActions.forEach(
                action -> {
                    if (impactAssessor.isImpactAligned(action, overallImpactAssessment)) {
                        ...
                        impactAssessor.undoActionImpactOnOverallAssessment(
                                action, overallImpactAssessment);
                    }
                });
    ...
    /**
     * A comparator for actions to sort them based on their impact from least pressure decreasing to
     * most.
     */
    @VisibleForTesting
    static final class ImpactBasedActionComparator implements Comparator<Action>, Serializable {
        ...
        public int compare(Action action1, Action action2) {
            int numberOfPressureReductions1 =
                    getImpactedDimensionCount(action1, Impact.DECREASES_PRESSURE);
            int numberOfPressureReductions2 =
                    getImpactedDimensionCount(action2, Impact.DECREASES_PRESSURE);

            if (numberOfPressureReductions1 != numberOfPressureReductions2) {
                return numberOfPressureReductions1 - numberOfPressureReductions2;
            }

            int numberOfPressureIncreases1 =
                    getImpactedDimensionCount(action1, Impact.INCREASES_PRESSURE);
            int numberOfPressureIncreases2 =
                    getImpactedDimensionCount(action2, Impact.INCREASES_PRESSURE);
