...
import com.amazon.opendistro.opensearch.performanceanalyzer.rca.Version;
...
public class QueryRcaRequestHandler extends MetricsHandler implements HttpHandler {
    ...
    private void handleLocalRcaRequest(
            final Map<String, String> params, final HttpExchange exchange) throws IOException {
        ...
        try {
            if (Stats.getInstance().getMutedGraphNodes().contains(rcaRequested)) {
                ...
                builder.append("The Rca '")
                        .append(rcaRequested)
                        .append(
                                "' is muted. Consider removing it "
                                        + "from the rca.conf's 'muted-rcas' list");
                errorResponse.addProperty("error", builder.toString());
                sendResponse(
                        exchange, errorResponse.toString(), HttpURLConnection.HTTP_BAD_REQUEST);
            } else {
    ...
    private JsonElement getRcaData(Persistable persistable, List<String> rcaList) {
        ...
        if (persistable != null) {
            rcaList.forEach(rca -> jsonObject.add(rca, persistable.read(rca)));
        }
        return jsonObject;
    }
