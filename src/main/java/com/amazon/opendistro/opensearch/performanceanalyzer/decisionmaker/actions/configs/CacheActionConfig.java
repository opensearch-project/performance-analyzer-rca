...
package com.amazon.opendistro.opensearch.performanceanalyzer.decisionmaker.actions.configs;
 ...
 *
 * <p>Configs are expected in the following json format: { "action-config-settings": { // Cache Max
 * Size bounds are expressed as %age of JVM heap size "cache-settings": { "total-step-count": 20,
 * "fielddata": { "upper-bound": 0.4, "lower-bound": 0.1 }, "shard-request": { "upper-bound": 0.05,
 * "lower-bound": 0.01 } } } }
 */
public class CacheActionConfig {
    ...
    public ThresholdConfig<Double> getThresholdConfig(ResourceEnum cacheType) {
        if (!thresholdConfigMap.containsKey(cacheType)) {
            String msg =
                    "Threshold config requested for unknown cache type: " + cacheType.toString();
            LOG.error(msg);
    ...
    private static class FieldDataCacheConfig implements ThresholdConfig<Double> {
        ...
        public FieldDataCacheConfig(NestedConfig cacheSettingsConfig) {
            NestedConfig fieldDataCacheConfig =
                    new NestedConfig("fielddata", cacheSettingsConfig.getValue());
            fieldDataCacheUpperBound =
                    new Config<>(
                            "upper-bound",
                            fieldDataCacheConfig.getValue(),
                            DEFAULT_FIELDDATA_CACHE_UPPER_BOUND,
                            (s) -> (s > 0),
                            Double.class);
            fieldDataCacheLowerBound =
                    new Config<>(
                            "lower-bound",
                            fieldDataCacheConfig.getValue(),
                            DEFAULT_FIELDDATA_CACHE_LOWER_BOUND,
                            (s) -> (s > 0),
                            Double.class);
        }
    ...
    private static class ShardRequestCacheConfig implements ThresholdConfig<Double> {
        ...
        public ShardRequestCacheConfig(NestedConfig cacheSettingsConfig) {
            NestedConfig shardRequestCacheConfig =
                    new NestedConfig("shard-request", cacheSettingsConfig.getValue());
            shardRequestCacheUpperBound =
                    new Config<>(
                            "upper-bound",
                            shardRequestCacheConfig.getValue(),
                            DEFAULT_SHARD_REQUEST_CACHE_UPPER_BOUND,
                            (s) -> (s > 0),
                            Double.class);
            shardRequestCacheLowerBound =
                    new Config<>(
                            "lower-bound",
                            shardRequestCacheConfig.getValue(),
                            DEFAULT_SHARD_REQUEST_CACHE_LOWER_BOUND,
                            (s) -> (s > 0),
                            Double.class);
        }
