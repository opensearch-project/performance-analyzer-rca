...
public class ImpactAssessment {
     ...
     */
    public void addActionImpact(
            @NonNull final String actionName, @NonNull final ImpactVector impactVector) {
                ...
                (dimension, impact) -> {
                    switch (impact) {
                        case INCREASES_PRESSURE:
                            addActionToMap(
                                    perDimensionPressureIncreasingActions, actionName, dimension);
                            break;
                        case DECREASES_PRESSURE:
                            addActionToMap(
                                    perDimensionPressureDecreasingActions, actionName, dimension);
                            break;
                        case NO_IMPACT:
                            break;
                        default:
                            LOG.warn(
                                    "Unknown impact value: {} encountered while adding action: {}'s impact",
                                    impact,
                                    actionName);
                    }
                });
     ...
     *     impacted by taking this action.
     */
    public void removeActionImpact(
            @NonNull final String actionName, @NonNull ImpactVector impactVector) {
        final Map<Dimension, Impact> impactMap = impactVector.getImpact();
        ...
        impactMap.forEach(
                (dimension, impact) -> {
                    switch (impact) {
                        ...
                        default:
                            LOG.warn(
                                    "Unknown impact value: {} encountered while removing action: {}'s impact",
                                    impact,
                                    actionName);
                    }
                });
     ...
     */
    public boolean checkAlignmentAcrossDimensions(
            @NonNull final String actionName, @NonNull final ImpactVector impactVector) {
        ...
        for (final Map.Entry<Dimension, Impact> entry : impactMap.entrySet()) {
            ...
            if (isAligned && impactOnDimension.equals(Impact.INCREASES_PRESSURE)) {
                ...
                if (!isAligned) {
                    LOG.info(
                            "action: {}'s impact is not aligned with node: {}'s overall impact for "
                                    + "dimension: {}. Found pressure decreasing actions: {}",
                            actionName,
                            nodeKey,
                            entry.getKey(),
                            perDimensionPressureDecreasingActions.getOrDefault(
    ...
    private void addActionToMap(
            @NonNull final Map<Dimension, List<String>> map,
    ...
    private void removeActionFromMap(
            @NonNull final Map<Dimension, List<String>> map,
            @NonNull final String actionName,
            @NonNull final Dimension dimension) {
