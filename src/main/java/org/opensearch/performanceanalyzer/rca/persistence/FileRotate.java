...
public class FileRotate {
     ...
     *
     * @return null if the file was not rotated because it is not old enough, or the name of the
     *     file after rotation.
     */
     ...
     *
     * <p>This can be called when say, the DB file is corrupted and we just want to rotate and
     * create a new file instead of throwing away data in this iteration.
     *
     * @return Path to the file after it is rotated or null if it ran into a problem trying to do
     *     so.
     */
    ...
    protected boolean shouldRotate(long currentTimeMillis) {
        ...
        long timeUnitsPassed =
                ROTATION_TIME_UNIT.convert(timeSinceLastRotation, TimeUnit.MILLISECONDS);
     ...
     * deleted either, we throw an IOException for the caller to handle or take the necessary
     * action.
     *
     * @return Returns the path to the file after it was rotated, this is so that the GC can add it
     *     to the list.
    ...
    protected synchronized Path rotate(long currentMillis) throws IOException {
        ...
        targetFileName
                .append(FILE_PART_SEPARATOR)
                .append(ROTATED_FILE_FORMAT.format(currentMillis));

        Path targetFilePath = Paths.get(dir, targetFileName.toString());

        Path ret = rotateFile(FILE_TO_ROTATE, targetFilePath);
    ...
    public static Path rotateFile(Path fileToRotate, Path targetFilePath) throws IOException {
        ...
        // try 2: Rotate the file now with replacement and add a log saying the destination file
        // will be deleted.
        // try 3: Delete the file, don't rotate. The caller will create a new file and start over.
        // try 4: If the delete fails, all bets are off, throw an exception and let the caller
        // decide.
        Path ret;
        try {
            ...
            LOG.error(
                    "**Deleting file '{}' or else we cannot rotate the current {}",
                    targetFilePath,
                    fileToRotate);
