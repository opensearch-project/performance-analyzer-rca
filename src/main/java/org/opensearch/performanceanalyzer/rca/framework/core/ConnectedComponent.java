...
public class ConnectedComponent {
    ...
    public List<List<Node<?>>> getAllNodesByDependencyOrder() {
        ...
        // Loop to initiate the ingressCountArray. For each node, as it is positioned in the
        // allNodes
        // list,
            ...
            for (Node<?> node : innerList) {
                ...
                for (Node<?> downstreamNode : node.getDownStreams()) {
                    ...
                    if (ingressCountArray[pos] == 0) {
                        // If decrementing the ingress, brings down the ingress to zero, then this
                        // is the new
                        // contender
    ...
    public static Set<String> getNodesForAllComponents(
            List<ConnectedComponent> connectedComponentList) {
