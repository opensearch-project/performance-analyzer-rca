...
public class OSEventProcessor implements EventProcessor {
    ...
    private Map<String, String> extrackKeyValFromData(String osMetricsData) {
        ...
        for (String line : lines) {
            ...
            osMetricsKeyValPairs.put(
                    pair[0], String.join(":", Arrays.copyOfRange(pair, 1, pair.length)));
        }
    ...
    private boolean processOSEvent(String data, String threadID) {
        ...
        if (opFileLastModified > endTime) {
            LOG.info(
                    "File last modified {} time is greater than endTime - {}",
                    opFileLastModified,
                    endTime);
        ...
        for (AllMetrics.OSMetrics metric : metrics) {
        ...
        String threadName =
                processedData.get(OSMetricsCollector.MetaDataFields.threadName.toString());
