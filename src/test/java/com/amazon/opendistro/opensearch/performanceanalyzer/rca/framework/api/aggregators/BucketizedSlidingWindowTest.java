...
public class BucketizedSlidingWindowTest {
    ...
    public void testBucketization() {
        BucketizedSlidingWindow slidingWindow =
                new BucketizedSlidingWindow(10, 2, TimeUnit.SECONDS);
        ...
        slidingWindow.next(
                new SlidingWindowData(TimeUnit.SECONDS.toMillis(currTimeInSecs + 10), 5));
        assertEquals(2, slidingWindow.size());
        ...
        slidingWindow.next(
                new SlidingWindowData(TimeUnit.SECONDS.toMillis(currTimeInSecs + 11), 6));
        assertEquals(1, slidingWindow.size());
        assertEquals(6, slidingWindow.readSum(), 0.00000001);
    }

    /**
     * When events are added frequently, with time between consecutive events being less than the
     * bucket window size, we should still see aggregated buckets across the full sliding window
     * timeline.
     */
    @Test
    ...
    public void testReadAvgWithPartialPruning() {
        BucketizedSlidingWindow slidingWindow =
                new BucketizedSlidingWindow(10, 1, TimeUnit.SECONDS);
        long currTimeInSecs = TimeUnit.MILLISECONDS.toSeconds(System.currentTimeMillis());
        slidingWindow.next(
                new SlidingWindowData(TimeUnit.SECONDS.toMillis(currTimeInSecs - 11), 11));
        ...
        assertEquals(
                3,
                slidingWindow.windowDeque
                        .size()); // we cannot call slidingWindow.size() as it also prunes expired
        // elements
    ...
    public void testReadTimeAvgWithPartialPruning() {
        ...
        // elements
        assertEquals(
                (double) (9 + 5) / (double) 2, slidingWindow.readAvg(TimeUnit.SECONDS), 0.00000001);
        assertEquals(2, slidingWindow.size());
    }
    ...
    public void testInit() {
        BucketizedSlidingWindow slidingWindow =
                new BucketizedSlidingWindow(10, 10, TimeUnit.SECONDS);
    }
}
