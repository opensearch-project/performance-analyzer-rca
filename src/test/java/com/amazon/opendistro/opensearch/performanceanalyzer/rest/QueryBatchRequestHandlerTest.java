...
import com.amazon.opendistro.opensearch.performanceanalyzer.metrics.MetricsRestUtil;
...
import com.amazon.opendistro.opensearch.performanceanalyzer.model.MetricsModel;
...
public class QueryBatchRequestHandlerTest {
    ...
    public void testHandle_invalidSamplingPeriod() throws Exception {
        ...
        checkBadQuery(
                queryPrefix
                        + "metrics=CPU_Utilization&starttime=1566413975000&endtime=1566413980000&samplingperiod=0");
        checkBadQuery(
                queryPrefix
                        + "metrics=CPU_Utilization&starttime=1566413975000&endtime=1566413980000&samplingperiod=4");
        checkBadQuery(
                queryPrefix
                        + "metrics=CPU_Utilization&starttime=1566413975000&endtime=1566413980000&samplingperiod=6");
        checkBadQuery(
                queryPrefix
                        + "metrics=CPU_Utilization&starttime=1566413975000&endtime=1566413980000&samplingperiod="
    ...
    public void testHandle_invalidTimes() throws Exception {
        ...
        checkBadQuery(
                queryPrefix
                        + "metrics=CPU_Utilization&starttime=1566413980000&endtime=1566413980000");
        checkBadQuery(
                queryPrefix
                        + "metrics=CPU_Utilization&starttime=1566413980000&endtime=1566413982000");
        checkBadQuery(
                queryPrefix
                        + "metrics=CPU_Utilization&starttime="
    ...
    public void testAppendMetrics() throws Exception {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            for (int j = 0; j < 8; j++) {
                ...
                if (metricAEnabled && !queryMetricA
                        || metricBEnabled && !queryMetricB
                        || metricCEnabled && !queryMetricC) {
                    continue;
                }
                ...
                Assert.assertEquals(
                        0,
                        handler.appendMetricsShim(
                                timestamp, metrics, builder, Integer.bitCount(i & j)));
    ...
    public void testQueryFromBatchMetrics_emptyBatchMetrics() throws Exception {
        ...
        String response =
                handler.queryFromBatchMetricsShim(
                        batchMetrics,
                        Arrays.asList("CPU_Utilization"),
                        timestamp,
    ...
    public void testQueryFromBatchMetrics_basic() throws Exception {
        ...
        String response =
                handler.queryFromBatchMetricsShim(
                        batchMetrics,
                        Arrays.asList("CPU_Utilization", "Paging_RSS", "Sched_Runtime"),
                        timestamp,
                        timestamp + MetricsConfiguration.SAMPLING_INTERVAL * 3,
                        5000,
                        9);
    ...
    public void testQueryFromBatchMetrics_missingTimestamps() throws Exception {
        ...
        NavigableSet<Long> batchMetrics = new TreeSet<Long>();
        List<Long> timestamps =
                Arrays.asList(timestamp, timestamp + MetricsConfiguration.SAMPLING_INTERVAL * 3);
        batchMetrics.addAll(timestamps);
        StringBuilder expectedResponseBuilder = new StringBuilder();
        expectedResponseBuilder.append("{");
        prepareMetricDB(timestamps.get(0), expectedResponseBuilder);
    ...
    public void testQueryFromBatchMetrics_missingTimestamps4() throws Exception {
        ...
        String response =
                handler.queryFromBatchMetricsShim(
                        ...
                        Arrays.asList("CPU_Utilization", "Paging_RSS", "Sched_Runtime"),
                        timestamp,
                        timestamp + MetricsConfiguration.SAMPLING_INTERVAL * 5,
                        10000,
                        9);
        Assert.assertEquals(expectedResponseBuilder.toString(), response);
    }
    ...
    private void prepareMetricDB(
            long timestamp,
            boolean metricAEnabled,
            boolean metricBEnabled,
            boolean metricCEnabled,
        ...
        List<String> dimensions =
                new ArrayList(MetricsModel.ALL_METRICS.get("CPU_Utilization").dimensionNames);
        if (metricAEnabled) {
            ...
            builder.append(
                    db.queryMetric(
                                    "CPU_Utilization",
                                    MetricsModel.ALL_METRICS.get("CPU_Utilization").dimensionNames,
                                    1)
                            .formatJSON());
        }
        if (metricBEnabled) {
            ...
            builder.append(
                    db.queryMetric(
                                    "Paging_RSS",
                                    MetricsModel.ALL_METRICS.get("Paging_RSS").dimensionNames,
                                    1)
                            .formatJSON());
        }
        if (metricCEnabled) {
            ...
            builder.append(
                    db.queryMetric(
                                    "Sched_Runtime",
                                    MetricsModel.ALL_METRICS.get("Sched_Runtime").dimensionNames,
                                    1)
                            .formatJSON());
        }
        builder.append("}");
