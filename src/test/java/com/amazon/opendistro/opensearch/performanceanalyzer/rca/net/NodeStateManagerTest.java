...
public class NodeStateManagerTest {
    ...
    public void updateReceiveTime() {
        ...
        testNodeStateManager.updateReceiveTime(
                new InstanceDetails.Id(TEST_HOST_1), TEST_NODE_1, currentTime);
        Assert.assertEquals(
                currentTime,
                testNodeStateManager.getLastReceivedTimestamp(
                        TEST_NODE_1, new InstanceDetails.Id(TEST_HOST_1)));
    }
    ...
    public void getLastReceivedTimestamp() {
        ...
        Assert.assertEquals(
                0,
                testNodeStateManager.getLastReceivedTimestamp(
                        TEST_NODE_1, new InstanceDetails.Id(TEST_HOST_2)));

        testNodeStateManager.updateReceiveTime(
                new InstanceDetails.Id(TEST_HOST_2), TEST_NODE_1, currentTime);
        Assert.assertEquals(
                currentTime,
                testNodeStateManager.getLastReceivedTimestamp(
                        TEST_NODE_1, new InstanceDetails.Id(TEST_HOST_2)));
    ...
    public void testNewNodesAddedToCluster() {
        ...
        testNodeStateManager.updateSubscriptionState(
                TEST_NODE_1,
        ...
        clusterDetailsEventProcessor.setNodesDetails(
                Lists.newArrayList(
                        ...
                        ClusterDetailsEventProcessorTestHelper.newNodeDetails(
                                TEST_HOST_1, IP_1, false),
                        ClusterDetailsEventProcessorTestHelper.newNodeDetails(
                                TEST_HOST_2, IP_2, false)));

        testNodeStateManager
                .getAppContext()
                .setClusterDetailsEventProcessor(clusterDetailsEventProcessor);

        ImmutableList<InstanceDetails> hostsToSubscribeTo =
                testNodeStateManager.getStaleOrNotSubscribedNodes(
                        TEST_NODE_1,
                        TEN_S_IN_MILLIS,
        ...
        clusterDetailsEventProcessor.setNodesDetails(
                Lists.newArrayList(
                        ...
                        ClusterDetailsEventProcessorTestHelper.newNodeDetails(
                                TEST_HOST_2, IP_2, false),
                        ClusterDetailsEventProcessorTestHelper.newNodeDetails(
                                TEST_HOST_3, IP_3, false)));
        testNodeStateManager
                .getAppContext()
                .setClusterDetailsEventProcessor(clusterDetailsEventProcessor);

        ImmutableSet<InstanceDetails.Id> hostSet =
                ImmutableSet.of(
                        new InstanceDetails.Id(
                                TEST_HOST_1), // We will send subscription as it has become stale.
                        new InstanceDetails.Id(
                                TEST_HOST_2), // We don't expect to send subscription to host2 as we
                        // received one recently.
                        new InstanceDetails.Id(
                                TEST_HOST_4)); // Although this is in the publisher list but not in
        // ClusterDetails, so we will
        // exclude it
        ImmutableList<InstanceDetails> hostsToSubscribeTo =
                testNodeStateManager.getStaleOrNotSubscribedNodes(
                        TEST_NODE_1, TEN_S_IN_MILLIS, hostSet);

        Assert.assertEquals(2, hostsToSubscribeTo.size());

        Set<String> expectedIs =
                hostsToSubscribeTo.stream()
                        .map(InstanceDetails::getInstanceId)
                        .map(InstanceDetails.Id::toString)
