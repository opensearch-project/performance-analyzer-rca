...
public class PerformanceAnalyzerWebServerTest {
    ...
    public void setup() {
        ...
        oldBindHost =
                PluginSettings.instance()
                        .getSettingValue(PerformanceAnalyzerWebServer.WEBSERVICE_BIND_HOST_NAME);
        oldPort = PluginSettings.instance().getProperty(PluginSettings.WEBSERVICE_PORT_CONF_NAME);
        oldCertificateFilePath =
                PluginSettings.instance().getSettingValue(CertificateUtils.CERTIFICATE_FILE_PATH);
        oldPrivateKeyFilePath =
                PluginSettings.instance().getSettingValue(CertificateUtils.PRIVATE_KEY_FILE_PATH);
        oldTrustedCasFilePath =
                PluginSettings.instance().getSettingValue(CertificateUtils.TRUSTED_CAS_FILE_PATH);
        oldClientCertificateFilePath =
                PluginSettings.instance()
                        .getSettingValue(CertificateUtils.CLIENT_CERTIFICATE_FILE_PATH);
        oldClientPrivateKeyFilePath =
                PluginSettings.instance()
                        .getSettingValue(CertificateUtils.CLIENT_PRIVATE_KEY_FILE_PATH);
        oldClientTrustedCasFilePath =
                PluginSettings.instance()
                        .getSettingValue(CertificateUtils.CLIENT_TRUSTED_CAS_FILE_PATH);
        oldHttpsEnabled = PluginSettings.instance().getHttpsEnabled();
        // Update bind host, port, and server certs for the test
        PluginSettings.instance()
                .overrideProperty(
                        PerformanceAnalyzerWebServer.WEBSERVICE_BIND_HOST_NAME, BIND_HOST);
        PluginSettings.instance().overrideProperty(PluginSettings.WEBSERVICE_PORT_CONF_NAME, PORT);
    ...
    public void tearDown() {
        ...
        if (oldBindHost != null) {
            PluginSettings.instance()
                    .overrideProperty(
                            PerformanceAnalyzerWebServer.WEBSERVICE_BIND_HOST_NAME, oldBindHost);
        } else {
            PluginSettings.instance()
                    .overrideProperty(
                            PerformanceAnalyzerWebServer.WEBSERVICE_BIND_HOST_NAME, "localhost");
        }
        if (oldPort != null) {
            PluginSettings.instance()
                    .overrideProperty(PluginSettings.WEBSERVICE_PORT_CONF_NAME, oldPort);
        } else {
            PluginSettings.instance()
                    .overrideProperty(PluginSettings.WEBSERVICE_PORT_CONF_NAME, "9600");
        }
        if (oldCertificateFilePath != null) {
            PluginSettings.instance()
                    .overrideProperty(
                            CertificateUtils.CERTIFICATE_FILE_PATH, oldCertificateFilePath);
        }
        if (oldPrivateKeyFilePath != null) {
            PluginSettings.instance()
                    .overrideProperty(
                            CertificateUtils.PRIVATE_KEY_FILE_PATH, oldPrivateKeyFilePath);
        }
        if (oldTrustedCasFilePath != null) {
            PluginSettings.instance()
                    .overrideProperty(
                            CertificateUtils.TRUSTED_CAS_FILE_PATH, oldTrustedCasFilePath);
        } else {
    ...
    public void initializeServer(boolean useHttps) {
        ...
        server =
                PerformanceAnalyzerWebServer.createInternalServer(
                        Integer.parseInt(PORT), BIND_HOST, useHttps);
        Assert.assertNotNull(server);
        server.setExecutor(Executors.newFixedThreadPool(1));
        // Setup basic /test endpoint. When the server receives any request on /test, it responds
        // with "hello"
    ...
    /**
     * Issues a basic HTTP GET request to $BIND_HOST:$PORT/test and verifies that the response says
     * "hello"
     */
    ...
    /**
     * Issues a basic HTTPS GET request to $BIND_HOST:$PORT/test and verifies that the response says
     * "hello"
     */
    public void verifyHttpsRequest(
            String urlString, String clientCert, String clientKey, String clientCA)
            throws Exception {
        // Build the request
        URL url = new URL(urlString);
    ...
    /**
     * testHttpServer verifies that any client can issue HTTP requests to the {@link
     * PerformanceAnalyzerWebServer} when TLS is disabled
     */
    @Test
    ...
    /**
     * Verifies that the server accepts any client's requests when HTTPS is enabled but Auth is
     * disabled.
     */
    @Test
     ...
     * @param clientCA Client certificate authority (used to verify server identity) Set this to
     *     null if you don't want to authenticate the server
    ...
    public SSLSocketFactory createSSLSocketFactory(
            String clientCert, String clientKey, @Nullable String clientCA) throws Exception {
        ClassLoader classLoader = getClass().getClassLoader();
        ...
        String prevClientCA =
                instance.getSettingValue(CertificateUtils.CLIENT_TRUSTED_CAS_FILE_PATH);
        // Override client identity settings
        instance.overrideProperty(
                CertificateUtils.CERTIFICATE_FILE_PATH,
        ...
        instance.overrideProperty(CertificateUtils.CERTIFICATE_FILE_PATH, certFile);
        instance.overrideProperty(CertificateUtils.PRIVATE_KEY_FILE_PATH, pKey);
