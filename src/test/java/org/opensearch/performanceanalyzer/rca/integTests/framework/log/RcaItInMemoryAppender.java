...
@Plugin(
        name = RcaItInMemoryAppender.NAME,
...
public class RcaItInMemoryAppender extends AbstractAppender {
    ...
    public static final String PATTERN =
            "%d{HH:mm:ss.SSS} [%t] %-5level %logger{1}:%M()::line:%L - %msg%n";
    ...
    private RcaItInMemoryAppender(
            final String name, final Layout<?> layout, final Property[] properties) {
        super(name, null, layout, true, properties);
    }

    /** Creates a CountingNoOp Appender. */
    @PluginFactory
    public static RcaItInMemoryAppender createAppender(
            @PluginAttribute("name") final String name, @PluginElement("Layout") Layout layout) {
        self =
                new RcaItInMemoryAppender(
                        Objects.requireNonNull(name), layout, Property.EMPTY_ARRAY);
        return self;
    }
