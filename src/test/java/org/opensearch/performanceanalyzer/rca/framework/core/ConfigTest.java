...
public class ConfigTest {
    ...
    public void testConfig() {
        Config<String> testKeyString =
                new Config<>("test-key-string", testConfig.getValue(), "default-val", String.class);
        assertEquals("test-key-string", testKeyString.getKey());
        assertEquals("value-1", testKeyString.getValue());

        Config<Double> testKeyDouble =
                new Config<>("test-key-double", testConfig.getValue(), 0.9, Double.class);
        assertEquals("test-key-double", testKeyDouble.getKey());
        assertEquals(0.5, testKeyDouble.getValue(), 0.000001);

        Config<Integer> testKeyInt =
                new Config<>("test-key-int", testConfig.getValue(), 11, Integer.class);
    ...
    public void testDefaults() {
        Config<String> testString =
                new Config<>("random-key", testConfig.getValue(), "default-val", String.class);
    ...
    public void testTypeMismatch() {
        Config<Double> testKey =
                new Config<>("test-key-string", testConfig.getValue(), 0.15, Double.class);
        assertEquals(
                0.15,
                testKey.getValue(),
                0.000001); // In case of type mismatch, we use default value
    }
    ...
    public void testInvalidConfig() {
        Config<Integer> test =
                new Config<>(
                        "test-key-int", testConfig.getValue(), 15, (v) -> (v >= 10), Integer.class);
